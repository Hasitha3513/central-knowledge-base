...
Mapping, batch, correction, approval, and export replays are safe under concurrent use. Validation rejects a canonical export payload above 32 KiB before release. A controlled JSON file is deterministic and its exact UTF-8 bytes match the displayed Integration SHA-256 evidence; a correction retains its original-line reference and never rewrites the released predecessor.
