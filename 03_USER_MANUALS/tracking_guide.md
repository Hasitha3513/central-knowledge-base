...
Authorized operators with `GPS_EXCEPTION_VIEW` can read bounded same-Tenant episodes and minimized evidence
through the API. Operators with `GPS_EXCEPTION_REVIEW` can acknowledge an open episode using a safe review
reason and idempotency key; retry returns the original successful response without another audit or side effect.
The reason is not returned or copied into audit.

Open **Tracking → GPS Exceptions** with `GPS_EXCEPTION_VIEW`. Choose a UTC From and To value no more than seven
days apart, then select **Load exceptions**. Optional status, type, severity, Vehicle and device filters narrow
the server result; **Back** and **Next** follow the server's opaque cursor. Selecting an episode opens its safe
detail and immutable evidence history. Filters are not copied into the URL or retained after sign-out.

Users who also hold `GPS_EXCEPTION_REVIEW` may acknowledge an eligible open episode. Enter a meaningful reason
of 1–500 characters and confirm once. Keep the dialog open when delivery is uncertain and use its retry action;
the client safely reuses the original command identity. If another operator changes the episode first, the page
