...
US-49 geofence management, Notification integration and operator UI are available and independently accepted. Speed detection, management/query API, Dispatcher IN_APP Notification integration and the dedicated rule/state/episode operator frontend are implemented. Route-deviation detection, management/query/review backend API and Dispatcher IN_APP Notification integration are implemented; its operator frontend remains a later change set. Journey replay point and stop query APIs are available to users with `JOURNEY_REPLAY_VIEW`; incident queries additionally require `JOURNEY_REPLAY_INCIDENT_VIEW` and remain unavailable until the governed incident-adapter change set. The journey replay operator UI is not yet available. Idle detection, the full tracking dashboard, GPS exception workflows and customer location exposure are not available. Automatic retention purge is disabled; configuring a retention policy enables too-old rejection/metadata but does not schedule purge. US-48 and final US-50 fidelity acceptance still require a physical GPS device and verified real provider speed payload.
