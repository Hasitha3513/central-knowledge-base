...
US-49 geofence backend management, Notification integration, operator UI, and concurrency/performance technical closure are available; independent final acceptance remains. Speeding, idle detection, route deviation, journey replay, the full tracking dashboard, GPS exception workflows and customer location exposure are not available. Automatic retention purge is disabled; configuring a retention policy enables too-old rejection/metadata but does not schedule purge. US-48 final acceptance still requires a physical GPS device and real provider payload.
