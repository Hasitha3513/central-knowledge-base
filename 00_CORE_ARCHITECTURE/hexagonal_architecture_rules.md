...
P0-07 introduced no broker, distributed transaction, saga, or database schema. P1-01 subsequently approved one shared PostgreSQL outbox for the Delivery-to-Notification US-69 family, with per-Tenant stable event identity, bounded retry/retention, explicit version handling, and Notification execution-key dedupe. Other local Spring events remain non-durable. Any future durable or external family still requires explicit contract approval; no general broker, global ordering, or exactly-once semantics are approved.
