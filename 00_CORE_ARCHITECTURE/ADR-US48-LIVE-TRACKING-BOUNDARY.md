...
Tracking remains the owner. CS01 has implemented the provider-neutral `TrackingProviderAdapter` outbound SPI, immutable `TrackingProviderAdapterRegistry`, strict `ProviderType`, exact immutable capability catalogue and bounded provider-neutral DTOs. Duplicate provider types fail startup, unsupported providers/discovery fail safely, and architecture tests prevent the SPI from depending on Spring, persistence, Jackson or Tracking adapters. Vendor DTOs, Tenant claims and secrets remain adapter-private. V75 implements the provider binding as the sole runtime provider-connection aggregate. V76 implements exactly one Tracking-owned `tracking_device_provider_binding` table as the sole device/connection/external-identity authority, with same-Tenant constraints, deterministic fail-closed legacy backfill, atomic history-preserving rebind, bounded non-secret configuration, watermarks, scheduling cursor and optimistic concurrency; V1–V75 remain immutable. Legacy device provider fields are compatibility projections only. Secrets remain opaque references resolved by `IntegrationSecretResolver`; no per-device environment variables are permitted.

CS04 implements one feature-flagged database-discovered coordinator, disabled by default until CS05. It claims bounded batches of due ACTIVE connections with `FOR UPDATE SKIP LOCKED`, owner-safe PostgreSQL leases, bounded fixed workers/queue, connection single-flight and provider-type quotas; no scheduler/thread exists per device. Activation and disable take effect without restart. Internal polling adapters use the private provider-neutral `TrackingProviderIngestionPort`, which reloads the leased ACTIVE connection, trusted Tenant, ACTIVE device binding and device lifecycle before delegating to the existing normalized ingestion service. Watermarks advance only for accepted or exact-duplicate facts; rejected/failed work remains retryable. The signed HMAC/nonce endpoint remains unchanged for external push providers. Neither path accepts payload Tenant authority.

CS05 completes the single-runtime Flespi cutover. Exactly one `TrackingProviderAdapter` implements `ProviderType("FLESPI")` and is dispatched through the registry/coordinator/internal-ingestion chain. The singleton scheduled poller, device-specific runtime properties, in-memory watermark authority and self-HTTP HMAC bridge are removed. The adapter consumes persisted connection execution plus bounded V76 device cursors, resolves no secret itself, supports multiple connections/Tenants/devices through bounded sequential provider calls, and returns only normalized candidates/watermark proposals. The unchanged signed endpoint remains available solely for external push providers. Rollback disables the coordinator and affected runtime connections; it never reactivates dual polling.

CS06 implements the provider-neutral management APIs under the existing `/api/v1/tracking` base. Installed provider descriptors, Tenant-scoped connection create/list/get/update/test/lifecycle, capability-gated discovery and device bind/rebind are available without restart. Connection and device creation default to DRAFT; both use the frozen four-state lifecycle with RETIRED terminal. Adapter validation precedes persistence, secrets remain opaque resolver-backed references, transient secret buffers are cleared, and responses expose only safe configuration and configured/masked state. Existing position/history, association, ingestion and real-device contracts remain unchanged. Management reuses `TRACKING_DEVICE_MANAGE`. Accounting remains 72/87; US-49 remains blocked; CS07 is next.
