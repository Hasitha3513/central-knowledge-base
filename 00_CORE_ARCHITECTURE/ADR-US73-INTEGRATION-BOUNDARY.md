...
- **Status:** Accepted product decision; `FROZEN_NOT_IMPLEMENTED_US73`
...
1. Create a dedicated top-level `integration` bounded context when US-73 implementation begins. Integration owns endpoint configuration, declarative mapping, external exchange state/attempts, health, and Integration audit records. It does not own or decide business facts.
...
12. No database migration number is reserved by this decision. US-73 implementation must use the next actual Flyway head after V60 and must not modify historical migrations.
