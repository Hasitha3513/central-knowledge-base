...
The correction requester must differ from its approver. Existing US-35 importer-versus-reconciler separation remains enforced and cannot be bypassed through US-38. Existing Audit access remains authoritative, so no `FUEL_EXCEPTION_AUDIT_VIEW` is introduced. Tenant comes only from trusted server/worker context; foreign IDs are safe not-found. No permission grants source-row mutation, fraud/theft determination, punishment, payment, provider authority, arbitrary Operations-case creation, or cross-Tenant access. Status is `PRODUCT_DECISIONS_FROZEN / NOT_SEEDED`.
