...
- `TRACKING_DEVICE_MANAGE`: same-Tenant device-reference lifecycle, effective-dated Vehicle association, and the authorized-but-not-yet-implemented provider-connection/device-binding create, update, test, discovery, activate, disable, retire and rebind operations. No separate credential/configuration permission is authorized; APIs return only configured/masked reference state and never secret values.
