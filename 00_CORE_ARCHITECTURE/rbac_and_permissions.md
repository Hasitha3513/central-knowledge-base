...
- `TRACKING_DEVICE_MANAGE`: same-Tenant device-reference lifecycle, effective-dated Vehicle association, and implemented provider-connection/device-binding create, update, test, discovery, activate, disable, retire and rebind operations. HTTP and method security cover the literal `/api/v1/tracking` management routes. No separate credential/configuration permission is authorized; APIs return only configured/masked state and never secret values.
