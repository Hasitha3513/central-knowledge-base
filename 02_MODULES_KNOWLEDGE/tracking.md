...
US-48 is `IMPLEMENTATION_COMPLETE / ACCEPTANCE_BLOCKED_EXTERNAL_SYSTEM`; pluggable-onboarding CS01–CS05 are complete and independently verified at V76, including the single-runtime Flespi SPI cutover. Tracking is a dedicated top-level bounded context for provider-neutral live Vehicle position facts. The trusted provider/Tenant authority, retention, observability, audit and rebuild remediation is implemented. Accounting remains 72/87 with 15 remaining; CS06 management APIs and physical-device/real-provider final acceptance are still required.
...
`US-48-PLUGGABLE-DEVICE-ONBOARDING-ARCHITECTURE-001` approves `PLUG_AND_PLAY_FOR_SUPPORTED_ADAPTERS`. Runtime administrators may onboard many devices and provider connections across Tenants without restart once the provider adapter is installed. New proprietary protocol code still requires a reviewed deployment; dynamic JAR upload is prohibited. CS01–CS05 are COMPLETE: provider SPI/registry, connection persistence, device binding persistence, coordinator and Flespi SPI cutover. CS06 management APIs are next.
...
Provider-connection/device-binding persistence and the provider execution coordinator are implemented at current head V76; management APIs and UI remain authorized but not implemented. V76 creates only `tracking_device_provider_binding` with same-Tenant device/provider foreign keys, Tenant-scoped external-reference uniqueness, one ACTIVE binding per device, the four-state lifecycle, bounded non-secret JSON configuration, execution watermarks, next-poll cursor, audit facts and optimistic version. CS04 added no migration, table, public API, permission or event. Its PostgreSQL proofs cover owner-safe claims/recovery, hot lifecycle changes, 100-device fair batching, multiple Tenants/connections, rebind/watermark races and index-backed discovery at 10,000 bindings. Real FMC130/Flespi evidence remains mandatory. Accounting stays 72/87; US-49 remains blocked.
