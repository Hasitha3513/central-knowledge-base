...
`US-48-FLESPI-LEVEL1-INBOUND-ADAPTER-AUTHORIZATION-001` authorizes one provider-specific adapter owned entirely by Tracking under `com.transportlogistics.app.tracking.adapters.inbound.flespi`; implementation has not started. Its initial transport is bounded HTTPS REST polling, not MQTT/webhook, with a five-second minimum/default interval, at most 500 messages per page, a bounded five-minute cold-start overlap and bounded transient retry/backoff. Provider DTOs, cursors and mappings remain private to the adapter.

The adapter resolves the least-privilege flespi token through the existing binding's opaque `credentialReference` and `IntegrationSecretResolver`. The same high-entropy resolved secret signs a private/local HTTP invocation of the unchanged `POST /api/integration/v1/tracking/positions` contract. Direct `TrackingUseCase` invocation is not authorized because the existing binding, HMAC, nonce and admission chain remains authoritative. No new public API, permission, event, table, migration, domain/application contract or Integration telemetry path is authorized.
...
US-48 remains `ACCEPTANCE_BLOCKED_EXTERNAL_SYSTEM`; accounting remains 72/87 complete with 15 remaining. Physical hardware, provider activation and real telemetry are still required. The next task is `US-48-FLESPI-LEVEL1-INBOUND-ADAPTER-IMPLEMENTATION-001`; US-49 must not start.
