...
## US-47 Implemented Business Family

US-47 implements `TransportBillingExportRequestedV1` / `TRANSPORT_BILLING_V1` as a `FINANCIAL_CONFIDENTIAL` P1-01 input to `integration-outbound-exchange`. Integration owns configuration, declarative mapping, controlled `FILE_JSON_V1` delivery, attempts, retry, hash evidence, health and technical audit; Billing retains all business meaning. The explicit allow-list, contract/privacy/size controls, real canonical file and recorded SHA-256 evidence pass. No live accounting provider or inbound acknowledgement exists, and successful delivery is never represented as posted, booked, settled or paid.
