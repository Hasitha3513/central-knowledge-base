...
MVP 1.4 Last-Mile Delivery: 8/8 COMPLETE (CLOSED). Overall register: 69/87 COMPLETE and 18/87 REMAINING after US-35 final acceptance.
...
| Table | Purpose | Primary key | Key columns | Constraints / indexes |
| :--- | :--- | :--- | :--- | :--- |
| `vehicle_reading` | Immutable odometer/hour reading | `reading_id UUID` | `tenant_id`, vehicle, type, value/unit, epoch, source/reference, recorded/received, actor, correction fields, idempotency, notes, created | type/unit/source/value checks; self correction FK; unique idempotency/correction; chronology/source indexes; V84 partial unique `(tenant_id, vehicle_id, reading_type, source_type, source_reference_id)` where source reference is non-null and correction is null |
...
### US-46 Implementation — Acceptance Pending

US-46 is implemented inside Fleet/Driver through V68–V70. The feature owns tenant-scoped `driver_payroll_input_batch`, `driver_payroll_input_line`, `driver_payroll_worker_mapping`, and `driver_payroll_input_history`; all primary identities are UUIDs, all tenant-owned uniqueness and lookup paths lead with `tenant_id`, batch and mapping writes use optimistic versions, released lines are trigger-protected from update/delete, and correction/original references are internal composite tenant foreign keys. `trip_id`, `driver_id`, actors, and Integration configuration remain logical cross-module UUID references without cross-module physical foreign keys.

The exact implemented columns are recorded by immutable migrations V68–V70: batch identity/type/correction reference, period/cutoff/currency/lifecycle, five monetary totals, preparer/approver timestamps, stable configuration/event/hash/idempotency/version/audit fields; line identity/batch/Driver/Trip snapshot, category/reason/description, quantity/unit/rate/amount, original line/hash/mapping snapshot; worker mapping identity/Driver/system alias/opaque worker reference/active/version/audit fields; and history identity/batch/action/from/to/actor/detail/time. The API, four permissions, deterministic payload, P1-01 event, controlled US-73 file delivery, and Driver Payroll UI are active pending independent technical closure. Verified implementation evidence is Maven 1,359/0/0/15, architecture 46/46, frontend 263/263, and real PostgreSQL-backed Chromium 6/6. Accounting remains 70/87; next task is `US-46-DRIVER-PAYROLL-LINK-TECHNICAL-CLOSURE-001`.

Technical closure subsequently identified durable worker-mapping idempotency, successful-delivery projection, validation-time payload sizing, complete correction/Tenant coverage, and closure-specific PostgreSQL evidence gaps. `US-46-TECHNICAL-REMEDIATION-AUTHORIZATION-001` approves V71, verified free, only for the minimal Tenant-scoped mapping-command idempotency schema and requires the remaining code/test remediation without new routes, permissions, outboxes, or payroll semantics. V1–V70 remain immutable; US-46 remains `IMPLEMENTATION_COMPLETE / ACCEPTANCE_PENDING`; accounting remains 70/87. Next task: `US-46-DRIVER-PAYROLL-LINK-TECHNICAL-REMEDIATION-001`.

### US-46 V71 Technical Remediation — Complete / Acceptance Pending

V71 adds `driver_payroll_worker_mapping_command` as a Driver/Fleet-owned, tenant-scoped append-only idempotency record. It carries UUID identity, `tenant_id`, textual `idempotency_key`, canonical `request_hash`, result mapping identity and safe result snapshot fields, actor UUID, and `created_at`; tenant-leading uniqueness is `(tenant_id, idempotency_key)`, lookup indexes remain tenant-leading, and its only physical relationship is the same-module composite Tenant/mapping reference. No Trip, Identity, Integration, HRMS, Finance, or other cross-module physical foreign key exists.

The remediation serializes batch command keys and released-source approval claims with PostgreSQL advisory transaction locks, preserves `SUPERSEDED` releases in duplicate-source exclusion, validates correction original lines against the same Tenant and predecessor, makes correction replay atomic, and re-canonicalizes stored Integration JSONB before controlled-file output so the written UTF-8 bytes match the stored SHA-256. Fresh acceptance-database evidence passed the V71/atomicity/isolation suite 18/18, exact PostgreSQL races 9/9, Maven 1,380/0/0/15, architecture 46/46, frontend 263/263, and PostgreSQL-backed Chromium 7/7. No public API, route, permission, outbox, module, payroll semantic, or migration beyond V71 changed. US-46 remains `IMPLEMENTATION_COMPLETE / ACCEPTANCE_PENDING`; accounting remains 70/87. Next task: `US-46-DRIVER-PAYROLL-LINK-TECHNICAL-CLOSURE-001-RERUN`.

`US-46-DRIVER-PAYROLL-LINK-TECHNICAL-CLOSURE-001-RERUN` passes independently. Fresh closure evidence is PostgreSQL 18/18 with deterministic concurrency 9/9 and clean Flyway V1→V71; Maven 1,380 tests with 0 failures, 0 errors, and 15 skips in 06:13; architecture 46/46; Checkstyle, PMD, SpotBugs, TypeScript, 263/263 Vitest, production build, and changed-file lint PASS; and real PostgreSQL-backed Chromium 7/7 in 39.2 seconds. Every authoritative database result used only `transport_logistics_acceptance`. US-46 remains acceptance pending, accounting remains 70/87, and the next task is `US-46-DRIVER-PAYROLL-LINK-FINAL-ACCEPTANCE-001`.

### US-46 Final Acceptance — Complete

`US-46-DRIVER-PAYROLL-LINK-FINAL-ACCEPTANCE-001` passes. Fresh evidence is PostgreSQL 18/18 with deterministic concurrency 9/9 and clean Flyway V1→V71; Maven 1,380 tests with 0 failures, 0 errors, and 15 skips in 06:21; architecture 46/46; all static/frontend gates including Vitest 263/263; and real PostgreSQL-backed Chromium 7/7 in 38.2 seconds. Driver/Fleet ownership, Payroll/HRMS authority, four categories, calculations, immutable correction/release behavior, durable mapping idempotency, Tenant/source isolation, P1-01 atomicity, truthful controlled-file `EXPORTED`, canonical file/hash, privacy, and four-permission RBAC all pass. No payroll engine, new route, permission, outbox, module, foreign persistence, distributed transaction, or migration beyond V71 exists. US-46 is `COMPLETE`; accounting is 71/87 with 16 remaining. Wave B remains open for US-47, and the next task is `US-47-TRANSPORT-BILLING-PRODUCT-DECISIONS-001`.

### Governed full-product completion plan

`DEFERRED-BACKLOG-REPRIORITIZATION-001` is complete as a planning task. Following US-46 final acceptance, story accounting is 71 / 87 accepted and 16 / 87 remaining. The exact remainder is US-47, US-48 through US-55, US-72, US-76, US-82, and US-84 through US-87. US-88, US-89, and US-90 remain undefined.

The remaining stories are reprioritized into governed waves: Wave A (US-73/78 integration and exception foundations) is 2/2 COMPLETE and CLOSED; Wave B is open with US-35, US-37, US-38, and US-46 complete and US-47 remaining; Wave C covers US-48..55 GPS/tracking; Wave D covers US-72/76 compliance/mobile; and Wave E covers US-85/84/87/82/86 integrity, resilience, risk, analytics, and disruption. The single next task is `US-47-TRANSPORT-BILLING-PRODUCT-DECISIONS-001`.
...
- Status: `COMPLETE / FINAL_ACCEPTANCE_PASS`; accounting is 69/87 accepted and 18/87 remaining. Wave B remains open. V64 is the US-35 migration; V65 is the current repository head and implements the authorized Bunker ledger-order remediation.
...
- The acceptance rerun then exposed inconsistent development/E2E sample facts: PostgreSQL seeded ten Tanks at 25,000 litres while their canonical DISPENSE tails were 24,920, and H2 had one nonzero Tank without history. `US-35-FUEL-CARDS-ACCEPTANCE-REMEDIATION-003` reconciles only those fixtures and adds post-provisioning invariant checks. Fresh evidence passed PostgreSQL/H2 provisioning 2/2, Fuel/Bunker/US-35/US-37 163/163, Maven 1,335/0/0/15, architecture 46/46, frontend 263/263, Chromium 6/6, and the post-E2E query returned zero missing histories and zero tail mismatches.
- Independent final acceptance rerun-2: PASS. Fresh focused US-35 passed 22/22; Bunker PostgreSQL concurrency passed 10/10; Fuel/Bunker/US-35/US-37 passed 163/163; Maven passed 1,335/0/0/15 in 05:09; architecture passed 46/46; all static-analysis gates passed; frontend passed TypeScript, Vitest 263/263, production build and scoped lint; real PostgreSQL Chromium passed 6/6; Flyway reached V65; and the immediate post-startup query returned zero nonzero history-free Tanks and zero ledger-tail mismatches. All authoritative database evidence used only `transport_logistics_acceptance`.
...
- Lifecycle/outcomes: `OPEN -> UNDER_REVIEW -> CORRECTION_PENDING -> AWAITING_APPROVAL -> RESOLVED`, with no generic patch/delete/cancel/reopen. Outcomes are no action, correction applied, reconciled, emergency refuel accepted, or referred to Operations. Handoff status is orthogonal and does not duplicate US-78.
- Corrections/SoD: immutable source history is corrected only through the owner command or compensating fact. Inventory, cost, price, reconciliation and source-lifecycle corrections require requester-distinct approval. US-35 importer/reconciler SoD remains intact. US-80 is not used; fixed Fuel transitions suffice.
- Bunker/price/emergency: negative stock is rejected and generates an exception; Tank correction uses canonical adjustment/`ledgerSequence`. Sudden price change creates a new effective period and never rewrites historical transactions. Emergency refuel requires reason plus Vehicle and Trip-or-Driver attribution; evidence may reference existing Fuel records/documents without inventing mandatory binary storage.
- Operations/P1-01: CRITICAL handoff is mandatory; qualified HIGH unresolved, safety/compliance, or cross-module cases may be escalated with reason. Fuel reuses `OperationalExceptionFactV1` and the shared durable outbox with stable handoff event identity, at-least-once retry and Operations dedupe. No second outbox/inbox, distributed transaction, Notification engine or generic Operations case model exists.
- Security/API/data: permissions are `FUEL_EXCEPTION_VIEW`, `MANAGE`, `CORRECT`, `APPROVE`, and `ESCALATE`; existing Audit access remains authoritative. Tenant is trusted server/worker context only. Planned `/api/v1/fuel/exceptions` APIs use bounded queries and explicit review/evidence/note/correction/approval/resolve/escalate commands. Expected Fuel-owned persistence is six Tenant-scoped aggregate/history tables with logical foreign references; no migration version is reserved.
- Acceptance: implementation must prove all six paths, immutable evidence/history, owner-command results, approval/SoD, dedupe/concurrency, durable handoff retry, privacy and Tenant A/B under PostgreSQL and real Chromium while preserving US-35, US-37, Bunker V65 and US-78/P1-01 behavior.

### Bunker Ledger Ordering Authorization

- `BUNKER-LEDGER-ORDERING-AUTHORIZATION-001` is APPROVED as non-story technical governance. It changed no Fuel Card contract or accounting; US-35 subsequently passed final acceptance at 69/87 accepted and 18/87 remaining.
...
`US-38-ACCEPTANCE-OBSERVABILITY-AUTHORIZATION-001` approves a test-only observability model for identities intentionally absent from the public API. Real Chromium must perform the ordinary user/API workflow; the same acceptance run may then issue read-only queries against exactly `transport_logistics_acceptance` to compare negative-Bunker `source_event_id`, immutable handoff event identity, Operations dedupe count, and correction-attempt idempotency. It may also confirm unchanged Tank stock, movement count, and ledger sequence.

Database assertions are supplemental and may not create, update, delete, repair, or invoke behavior. Credentials remain test-harness-side, are never bundled for the browser, hardcoded, or logged, and the harness must fail closed for any database other than `transport_logistics_acceptance`. No public DTO field, diagnostic route, permission, migration beyond V67, product semantic, event contract, outbox, or story-accounting change is authorized.
...
### US-47 Final Acceptance

`US-47-TRANSPORT-BILLING-FINAL-ACCEPTANCE-001` passes and US-47 is COMPLETE. Fresh final evidence is focused Billing 17/17, deterministic PostgreSQL concurrency 9/9, clean Flyway V1→V72, full Maven 1,398 tests with 0 failures and 0 errors (15 skipped) in 06:38, architecture 46/46, all static/frontend gates, and real PostgreSQL-backed Chromium 7/7 in 37.0 seconds. The dedicated Billing boundary, Finance authority, exact Trip/Freight eligibility, deterministic commercial facts, supplied-tax boundary, source/finalized immutability, exact reversal, durable idempotency, Tenant/RBAC controls, shared P1-01 outbox, and controlled US-73 file/hash delivery pass without scope leakage. Accounting advances once to 72/87 with 15 remaining; Wave B is COMPLETE/CLOSED and Wave C activates with US-48 product decisions.

### US-48 Live Vehicle Tracking Product Decisions

`US-48-LIVE-VEHICLE-TRACKING-PRODUCT-DECISIONS-001` is frozen with implementation not started. The dedicated provider-neutral `tracking` boundary owns narrow device references/effective-dated Vehicle association, signed HTTP JSON normalization, immutable WGS84 history, dedupe/order/trust, latest projections, explicit LIVE/RECENT/STALE/UNKNOWN freshness and connectivity, retention metadata and safe operator queries. Fleet/Trip/Routing/Organization ownership is preserved through logical same-Tenant references and published contracts. High-rate telemetry does not pass through US-73 or per-packet P1-01; ordinary PostgreSQL and 15-second UI polling are selected without PostGIS/Timescale/Kafka/Redis. Precise location is permission- and Tenant-restricted. Technical closure may use fixtures, but final acceptance requires a physical GPS device and real provider payloads or remains `BLOCKED_BY_EXTERNAL_SYSTEM`. Accounting remains 72/87; next task is `US-48-LIVE-VEHICLE-TRACKING-IMPLEMENTATION-001`.

### US-48 Live Vehicle Tracking Implementation

`US-48-LIVE-VEHICLE-TRACKING-IMPLEMENTATION-001` is `IMPLEMENTATION_COMPLETE / ACCEPTANCE_PENDING` at V73. The dedicated Tracking boundary, signed ingress, immutable history, effective-dated association, deterministic dedupe/order/trust/latest behavior, exact RBAC, minimal polling UI, 9/9 PostgreSQL races, 10/10 real Chromium controlled-provider suite, and load/query targets pass. No US-49–55 detector scope or per-packet event was introduced. Accounting remains 72/87 pending physical-device/real-provider acceptance; next is `US-48-LIVE-VEHICLE-TRACKING-TECHNICAL-CLOSURE-001`.
