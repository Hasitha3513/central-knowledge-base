...
US-69 Receive Delivery Notifications: IMPLEMENTATION_COMPLETE / ACCEPTANCE_BLOCKED — final acceptance found `DELIVERY_OUT_FOR_DELIVERY` published at Batch `READY` instead of committed `DISPATCHED`; the distinct dispatch path emits no customer event. Other technical closure evidence remains Maven 1,220/0/0/15, architecture 42/42, and real PostgreSQL-backed Chromium 6/6, but those suites did not cover this frozen trigger contract.
...
### US-69 Receive Delivery Notifications (Implementation Complete; Acceptance Blocked)
...
- **Technical verification**: Complete Maven passed 1,220 tests with 0 failures/errors and 15 skips; architecture passed 42/42; Checkstyle, PMD, and SpotBugs passed; frontend TypeScript, 57-file/254-test Vitest, production build, and changed-file lint passed; the real PostgreSQL-backed Chromium suite passed 6/6. Final acceptance nevertheless failed because these suites did not prove the out-for-delivery Batch trigger.
- **Acceptance blocker**: `DeliveryBatchService.markReady` publishes `DELIVERY_OUT_FOR_DELIVERY` for active members while committing Batch status `READY`. The frozen contract prohibits readiness-triggered notification and requires this event when `dispatchBatch` commits status `DISPATCHED`; that dispatch path currently publishes no customer event.
- **Program state**: US-69 is `IMPLEMENTATION_COMPLETE / ACCEPTANCE_BLOCKED`. MVP 1.4 remains 6/8, overall remains 63/87, and deferred remains 24/87. US-70 must not start until US-69 is remediated and independently reaccepted.
