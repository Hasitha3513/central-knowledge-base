...
| Last-Mile Planner (US-68) | `GET /v1/deliveries/{id}/last-mile-planner` | Delivery | Implemented read-only projection over existing order, attempt, escalation, and exception capabilities; requires `DELIVERY_FAIL_VIEW` or `DELIVERY_EXCEPTION_VIEW`; final acceptance pending |
