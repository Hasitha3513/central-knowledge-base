...
## US-49 Proposed Geofence Interfaces (Frozen; Not Implemented)

US-49 reserves a Tenant-scoped management/query family under `/api/v1/tracking/geofences`. CS04 must define exact request/response schemas without changing this base, expose explicit create/update/activate/disable/retire commands rather than arbitrary status mutation, and provide bounded definition and immutable transition queries. `GEOFENCE_VIEW`, `GEOFENCE_MANAGE` and `GEOFENCE_EVENT_VIEW` are the only approved human capabilities. Coordinates remain sensitive and never appear in cross-module transition events.

The Organization dependency now exposes a published explicit-Tenant location lookup equivalent to `find(tenantId, locationId)`, backed by tenant-scoped persistence for background evaluation safety. Tracking may store only the logical location UUID and cannot access Organization persistence. CS01's framework-neutral management, query, evaluation, persistence, job and transition-publication ports are `IMPLEMENTED`; the REST family remains `APPROVED / NOT_IMPLEMENTED` for a later slice.
