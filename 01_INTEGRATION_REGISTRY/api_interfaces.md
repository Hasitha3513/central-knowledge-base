...
The Organization dependency must evolve through a published, explicit-Tenant active-location lookup equivalent to `find(tenantId, locationId)` for background evaluation safety. Tracking may store only the logical location UUID and cannot access Organization persistence. These interfaces are `APPROVED_FOR_IMPLEMENTATION / NOT_IMPLEMENTED`; CS01 must first establish framework-neutral domain and ports.
