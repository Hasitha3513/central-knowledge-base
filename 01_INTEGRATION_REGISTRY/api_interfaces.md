...
| Fuel Cards (US-35, decisions frozen) | `/api/v1/fuel/cards`, card lifecycle/binding/restriction/history commands; `/api/v1/fuel/card-imports`; `/api/v1/fuel/card-transactions` plus explicit match/unmatch/reject commands | Fuel | `PRODUCT_DECISIONS_FROZEN / IMPLEMENTATION_NOT_STARTED`. Masked card-reference model; local-only lifecycle; controlled `FUEL_CARD_TRANSACTIONS_V1` JSON import; immutable provider facts; explicit reconciliation to an existing Fuel Purchase; no delete/raw edit/payment/provider-sync route. Default page 20, maximum 100. |
