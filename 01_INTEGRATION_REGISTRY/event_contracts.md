...
## VehicleGeofenceTransitionedV1 (US-49; Durable Tracking-to-Notification Contract Active)
...
CS05 atomically persists each alert-eligible transition and the canonical P1-01 outbox record through the
Tracking durable publisher adapter. The registered `geofence-transition-notification-bridge` consumer maps
the minimized version-1 payload to Notification's existing rule engine. Notification execution identity makes
at-least-once replay idempotent by Tenant, event, rule, channel and resolved recipient. Unauthorized-zone
confirmed entry uses `UNAUTHORIZED_ZONE_ENTERED` with HIGH severity and mandatory alert intent; it does not
create an Operations exception in US-49.
...
CS05 activation resolves that seeded Tenant rule and template to eligible same-Tenant Dispatchers. A clean
