...
| Tracking | US-51 | Authoritative engine-running plus movement telemetry | DECISION_READY_FOR_APPROVAL / IMPLEMENTATION_NOT_AUTHORIZED; software fixtures may proceed after approval, while production capability remains unavailable until a device-native source is verified |
...
The ARB disposition keeps US-48 physical acceptance `ON_HOLD_EXTERNAL_PREREQUISITE` until a genuine supported device/provider journey exists. The hold is not completion, acceptance or waiver. Downstream technical work may consume proven contracts without inheriting field acceptance. US-49 is accepted; US-50, US-52, US-53, US-54 and US-55 are technically complete with governed field/external holds. US-51 prerequisite/product-decision work is selected under `US-51-MONITOR-IDLE-TIME-PREREQUISITE-AND-PRODUCT-DECISIONS-001`; implementation remains unauthorized only until D1-D11 approval. Software fixtures do not require hardware, while production source activation still requires verified device-native engine-running evidence. Accounting remains 73/87 and Flyway is V100. The exact open US-55 acceptance queue is `US-55-HANDLE-GPS-EDGE-CASES-FINAL-ACCEPTANCE-001`.
