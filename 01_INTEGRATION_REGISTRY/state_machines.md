...
Provider connection test status is orthogonal: `NOT_TESTED`, `PASS`, `AUTH_FAILED`, `UNREACHABLE`, or `INVALID_CONFIGURATION`. It creates no telemetry state. The provider-connection lifecycle is `IMPLEMENTED_US48_V75`. Rebinding will disable the previous active device-provider binding and create the replacement atomically; it will never rewrite effective-dated Vehicle association or position history. Device-provider binding lifecycle and atomic rebinding remain `AUTHORIZED_NOT_IMPLEMENTED_US48_CS03`; forward-only V76 is authorized as their persistence prerequisite while V1–V75 remain immutable.
